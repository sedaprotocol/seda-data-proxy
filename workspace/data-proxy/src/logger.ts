...
import { Cause, Clock, Duration, Effect, FiberId, Inspectable, LogLevel, LogSpan, Logger, Match, Option } from "effect";
import type { DurationInput } from "effect/Duration";
import { LOG_FILE_DATE_PATTERN, LOG_FILE_DIR, LOG_FILE_MAX_FILES, LOG_LEVEL } from "./constants";
...
const parseRotatingFileConfig = (dirPath: string) =>
	Effect.gen(function* () {
		...
		const fileExists = yield* fs.exists(path.resolve(dirPath, ROTATED_FILE_CONFIG_FILE));
		...
		const rawConfig = yield* fs.readFileString(path.resolve(dirPath, ROTATED_FILE_CONFIG_FILE));
...
const rotatedFileLogger = <Message>(self: Logger.Logger<Message, string>, dirPath: string, options?: RotatedFileLoggerOptions) =>
	Effect.gen(function* () {
		const fs = yield* FileSystem.FileSystem;
		const path = yield* Path.Path;
		...
		const config: RotatedFileLoggerConfig = yield* parseRotatingFileConfig(absoluteDirPath);
		const lastLogFile = Option.fromNullable(config.files[config.files.length - 1]);
		...
		return yield* Logger.batched(self, options?.batchWindow ?? Duration.seconds(1), (output) =>
			Effect.gen(function* () {
				const now = yield* Clock.currentTimeMillis;
				const fileName = yield* createFileName(datePattern);
				const absoluteFilePath = path.resolve(absoluteDirPath, fileName);
				...
				if (openLogFile.filePath === absoluteFilePath) {
					...
					yield* fs.writeFileString(path.resolve(absoluteDirPath, ROTATED_FILE_CONFIG_FILE), JSON.stringify(config, null, 2));
...
const makeCustomLogger = Logger.make(({ logLevel, message, fiberId, date, annotations, cause, spans }) => {
...
export const logBootstrap =
	...
	<A, E, R>(self: Effect.Effect<A, E, R>) =>
		Effect.gen(function* () {
			...
			const fileLogger = yield* rotatedFileLogger(myStringLogger, yield* LOG_FILE_DIR, {
			...
			const combinedLoggerLive = Logger.replace(Logger.defaultLogger, combinedLogger);

			return yield* self.pipe(Logger.withMinimumLogLevel(LogLevel.fromLiteral(logLevel)), Effect.provide(combinedLoggerLive));
		});
