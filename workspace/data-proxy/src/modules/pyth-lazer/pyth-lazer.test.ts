...
describe("handleRequest latest_price surface (POST body)", () => {
	...
	it("fails a wait timeout with the missing feed id", async () => {
		const fake = makeFakeLazerClient();
		fakeHolder.current = fake;
		...
		await runWithTestClock(
			Effect.gen(function* () {
				const module = yield* ModuleService;
				yield* module.start();
				fake.deliverTick(fake.subscribeCalls[0].subscriptionId, [1], "100");
				...
				const fiber = yield* Effect.fork(
					module.handleRequest(
						...
						latestPriceBody({ priceFeedIds: [1, 999] }),
					),
				);
				yield* TestClock.adjust(Duration.millis(0));
				...
				expect(response.status).toBe(400);
				const body = (yield* Effect.promise(() => response.json())) as {
					data_proxy_error: string;
				};
				expect(body.data_proxy_error).toContain("Price feed not found: 999");
			}).pipe(
				Effect.provide(
					PythLazerModuleService(
						makeConfig({
