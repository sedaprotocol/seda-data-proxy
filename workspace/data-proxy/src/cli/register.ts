...
export const registerCmd = new Command("register")
	...
	.option("-pkf, --private-key-file <string>", "Path where to create the private key json")
	...
	.option("--memo <string>", "A custom note to attach to this Data Proxy registration")
	.action(async (adminAddress, fee, options) => {
		const program = Effect.gen(function* () {
			...
			if (options.network) {
				...
				const networkResult = yield* Effect.either(loadNetworkFromKeyFile(options.privateKeyFile));
				if (Either.isLeft(networkResult)) {
					yield* Effect.logError(`Failed to load network from private key file: ${networkResult.left}`);
					process.exit(1);
				}
			...
			const payoutAddress = Maybe.of(options.payoutAddress).unwrapOr(adminAddress);

			if (!isValidSedaAddress(adminAddress)) {
				yield* Effect.logError(`Admin address ${adminAddress} is not a valid SEDA address`);
				process.exit(1);
			}

			if (!isValidSedaAddress(payoutAddress)) {
				yield* Effect.logError(`Payout address ${payoutAddress} is not a valid SEDA address`);
				process.exit(1);
			}

			const aSedaAmount = trySync(() => sedaToAseda(fee)).map((amount) => `${amount}aseda`);
			...
			const hash = createHash(aSedaAmount.value, adminAddress, payoutAddress, memo, network.chainId);
			...
			yield* Effect.logInfo(`Fee amount: \t\t${fee} SEDA (${aSedaAmount.value})`);
		...
		}).pipe(Effect.provide(NodeFileSystem.layer), Logger.withMinimumLogLevel(LogLevel.Debug));
