...
const SRC_TARGET = [resolve(process.cwd(), "./workspace/data-proxy/src/index.ts")];
...
await Promise.all(
	PLATFORM_TARGETS.map(async (target) => {
		...
		const cmd = ["bun", "build", "--compile", `--target=${target}`, ...SRC_TARGET, "--outfile", `${BINARY_NAME}-${rawTarget}`];
